require_relative '../modules/initialization'
...
class Checkout
  ...
  def self.apply_discount(discount, items_data)
    ...
    item = discount['item']
    item_amount = items_data[item]['amount']
    type = discount['discount_type']
    affected_item = get_affected_item(discount)
    unless @@rule_map[type].nil? || items_data[affected_item].nil?
      # map with different discount func
      new_items_data[affected_item] = self.send(@@rule_map[type], discount['detail'], items_data[affected_item], item_amount)
    end
    new_items_data
  end
  ...
  def self.x_for_y(detail, item, _)
    new_item = item
    trigger_amount = detail['trigger_amount']
    discount_amount = detail['discount_amount']
    amount = item['amount']
    if amount >= trigger_amount
      new_item['amount'] = amount - (amount / trigger_amount).floor()
    end
    new_item
  end

  def self.subtract_price(detail, item, _)
    new_item = item
    trigger_amount = detail['trigger_amount']
    discount_amount = detail['discount_amount']
    amount = item['amount']
  ...
  def self.for_free(detail, item, free_amount)
    puts free_amount
    ...
    free_count = amount - discount_amount * free_amount
    new_item['amount'] = free_count >= 0 ? free_count : 0
    new_item
  end
